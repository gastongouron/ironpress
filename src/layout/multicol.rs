...
//! - `column-rule` painted as a vertical stroke centered in each gap.
...
pub(crate) fn layout_multicol_container(
    ...
    while i < items.len() {
        ...
        // Column rules sit in the gaps, spanning this run's column height.
    ...
    // the bottom padding.
    let explicit_h = style.height.map(|h| {
        if style.box_sizing == crate::style::computed::BoxSizing::BorderBox {
            h
        } else {
    ...
    let block_height = Some(explicit_h.unwrap_or(content_box_h));
...
/// Build a solid-color rule box spanning a column gap.
fn make_rule_container(
    ...
    empty_abs_container(Vec::new(), off_left, off_top, width, height, Some(color))
}
