use super::{
    CssValue, StyleMap, lightning::parse_inline_style_with_lightning, parse_length,
    spacing::parse_border_spacing_values, values::parse_property_value,
};
...
pub(super) fn apply_declaration(map: &mut StyleMap, raw_prop: &str, val: &str, is_important: bool) {
    ...
    if prop == "border-spacing" {
        ...
        if matches!(
            ...
            clear_border_spacing_entries(map);
            map.set_with_importance("border-spacing", CssValue::Keyword(lower), is_important);
            return;
        }
        let parsed: Option<(CssValue, CssValue)> = parse_border_spacing_values(val);
        if let Some((horizontal, vertical)) = parsed {
...
fn clear_border_spacing_entries(map: &mut StyleMap) {
    map.properties.remove("border-spacing-horizontal");
    map.important.remove("border-spacing-horizontal");
    map.properties.remove("border-spacing-vertical");
    map.important.remove("border-spacing-vertical");
}

fn reconcile_legacy_value_forms(parsed: &mut StyleMap, legacy: &StyleMap) {
    for (key, value) in &legacy.properties {
        if !parsed.properties.contains_key(key) || prefer_legacy_value_form(key) {
            parsed.set_with_importance(key, value.clone(), legacy.is_important(key));
        }
    }
}

fn prefer_legacy_value_form(key: &str) -> bool {
    matches!(
        key,
        "font-family"
            | "border"
...
mod tests {
    ...
    #[test]
    fn invalid_border_spacing_shorthand_is_ignored() {
        let style = parse_inline_style("border-spacing: 5pt 10pt 15pt");
    ...
    #[test]
    fn invalid_important_border_spacing_does_not_block_later_valid_value() {
        let style = parse_inline_style("border-spacing: 10% !important; border-spacing: 4px 8px");
        assert!(!style.is_important("border-spacing"));
        assert!(
            matches!(style.get("border-spacing"), Some(CssValue::Length(v)) if (*v - 3.0).abs() < 0.01)
        );
        assert!(
            matches!(style.get("border-spacing-horizontal"), Some(CssValue::Length(v)) if (*v - 3.0).abs() < 0.01)
        );
        assert!(
            matches!(style.get("border-spacing-vertical"), Some(CssValue::Length(v)) if (*v - 6.0).abs() < 0.01)
        );
    }

    #[test]
    fn parse_border_spacing_two_values_sets_synthetic_entries() {
        let style = parse_inline_style("border-spacing: 4px 8px");
        assert!(
            matches!(style.get("border-spacing-horizontal"), Some(CssValue::Length(v)) if (*v - 3.0).abs() < 0.01)
        );
        assert!(
    ...
    #[test]
    fn border_spacing_special_keywords_clear_synthetic_entries() {
        let style = parse_inline_style("border-spacing: 4px 8px; border-spacing: initial");
        assert!(matches!(
            style.get("border-spacing"),
    ...
    #[test]
    fn border_spacing_revert_clears_synthetic_entries() {
        let style = parse_inline_style("border-spacing: 4px 8px; border-spacing: revert");
        assert!(matches!(
            style.get("border-spacing"),
    ...
    #[test]
    fn border_spacing_rejects_invalid_shorthand_values() {
        let percentage = parse_inline_style("border-spacing: 10%");
        assert!(percentage.get("border-spacing").is_none());
        assert!(percentage.get("border-spacing-horizontal").is_none());
        assert!(percentage.get("border-spacing-vertical").is_none());

        let negative = parse_inline_style("border-spacing: calc(1pt - 2pt)");
        assert!(negative.get("border-spacing").is_none());
        assert!(negative.get("border-spacing-horizontal").is_none());
        assert!(negative.get("border-spacing-vertical").is_none());
    }
