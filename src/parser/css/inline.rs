use super::{
    ...
    parse_length,
    values::{BorderSpacingValue, parse_border_spacing_value, parse_property_value},
};
...
pub(super) fn apply_declaration(map: &mut StyleMap, raw_prop: &str, val: &str, is_important: bool) {
    ...
    if prop == "border-spacing" {
        if let Some(parsed) = parse_border_spacing_value(val) {
            match parsed {
                BorderSpacingValue::Keyword(keyword) => {
                    map.set_with_importance(
                        "border-spacing",
...
mod tests {
    ...
    #[test]
    fn parse_border_spacing_invalid_extra_token_is_ignored() {
        let style = parse_inline_style("border-spacing: 12pt bogus");
    ...
    #[test]
    fn parse_border_spacing_revert_keyword_is_preserved() {
        let style = parse_inline_style("border-spacing: revert");
        assert!(
            matches!(style.get("border-spacing"), Some(CssValue::Keyword(value)) if value == "revert")
        );
    }
