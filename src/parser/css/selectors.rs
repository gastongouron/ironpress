...
fn simple_selector_matches(
    ...
    if base.contains('[') {
        if let Some(bracket_index) = base.find('[') {
            ...
            let Some((attribute_sel, suffix)) = split_attribute_selector(attributes_sel) else {
                return false;
            };
            if !attribute_selector_matches(attribute_sel, attributes) {
                return false;
            }
            if suffix.is_empty() {
                return true;
            }
            return simple_selector_matches(suffix, tag, classes, id, attributes, ctx);
        }
    }
...
fn simple_selector_core_matches(
    ...
    fn tag_part_matches(tag_part: &str, tag: &str) -> bool {
        tag_part.is_empty() || tag_part == "*" || tag_part == tag
    }
    ...
    let mut tag_part_end = selector.len();
    ...
    while index < selector.len() {
        ...
        match ch {
            '.' | '#' => {
                tag_part_end = tag_part_end.min(index);
                let start = index + ch.len_utf8();
                let mut end = start;
                while end < selector.len() {
                    let Some(next) = selector[end..].chars().next() else {
                        break;
                    };
                    if matches!(next, '.' | '#') {
                        break;
                ...
                let token = selector[start..end].trim();
                if token.is_empty() {
                    return false;
                }
                match ch {
                    '.' => required_classes.push(token),
                    '#' => {
                        if required_id.replace(token).is_some() {
                            return false;
                        }
                    }
                    _ => unreachable!(),
    ...
    let tag_part = selector[..tag_part_end].trim();
    if !tag_part_matches(tag_part, tag) {
        return false;
    }
    if let Some(required_id) = required_id
        && !id.is_some_and(|value| value == required_id)
    {
        return false;
    }
    required_classes
        .iter()
        .all(|required_class| classes.iter().any(|class| class == required_class))
}
...
fn split_attribute_selector(selector: &str) -> Option<(&str, &str)> {
    ...
    let attribute = selector.get(start..=end)?;
    let suffix = selector.get(end + 1..)?.trim_start();
    Some((attribute, suffix))
}
